...
window.AtlasEdu = (function() {
    ...
    // Cross-level core modules
    const _TAHFIZH_TILE = { name: 'Tahfizh Center',      icon: 'fas fa-quran',        color: 'icon-gradient-indigo', action: 'EduTahfizh.openModule()' };
    const _RAPOR_TILE   = { name: 'Rapor Digital',        icon: 'fas fa-file-alt',     color: 'icon-gradient-teal',   action: 'EduStudentLifecycle.openRaporDigitalModal()' };
    const _CBT_TILE     = { name: 'CBT Exam Engine',     icon: 'fas fa-laptop-code',  color: 'icon-gradient-purple', action: 'EduStudentLifecycle.openCbtExamModal()' };
    const _IJAZAH_TILE  = { name: 'Graduation & Ijazah', icon: 'fas fa-certificate',  color: 'icon-gradient-amber',  action: 'EduStudentLifecycle.openIjazahModal()' };

    const unitModuleConfig = {
        preschool:  [
            _TAHFIZH_TILE, _RAPOR_TILE, _CBT_TILE, _IJAZAH_TILE,
    ...
    function _renderWorkspaceTiles(unitKey, unitName) {
                ...
                <div class="col-6 col-md-4 col-lg-3">
                    <div class="workspace-touch-tile" onclick="${onclick}">
                        ...
                        <div class="mt-auto pt-2 d-flex justify-content-between align-items-center">
                            ...
                            <span class="badge bg-success-subtle text-success" style="font-size:9px;">Isolated</span>
                        </div>
                    </div>
                </div>`;
            }).join('');
        ...
        if (window.AtlasFaultGuard) {
            window.AtlasFaultGuard.runIsolated('units', `Unit [${unitName}] Workspace`, moduleContainer, processRender);
        } else {
            processRender();
        }
    }
    ...
    function openModuleDetail(moduleName, unitName) {
        const renderDetail = () => {
            ...
            moduleScreen.innerHTML = `
                <div class="module-detail-card mt-3">
                    <div class="module-detail-header d-flex justify-content-between align-items-start">
                        <div>
                        ...
                        <button class="btn btn-sm btn-outline-danger fw-bold rounded-pill" onclick="AtlasFaultGuard.simulateFault('units')">
                            <i class="fas fa-bug me-1"></i> Test Fault Isolation
                        </button>
                    </div>
                    <div class="module-detail-body">
                        <div class="d-flex gap-2 flex-wrap mb-4">
                            ...
                            <span class="badge px-3 py-2 fw-semibold bg-dark text-white" style="font-size:11px;"><i class="fas fa-shield-alt text-teal me-1"></i>Circuit Guard Protected</span>
                        </div>

                        <h6 class="fw-bold text-muted mb-3" style="font-size:11px;letter-spacing:.06em;text-transform:uppercase;">Procedure Workflow Sandbox</h6>
                        ...
                        <div class="d-flex gap-2 flex-wrap">
                            <button class="btn-primary-atlas px-4 rounded-pill" style="font-size:13px;" onclick="AtlasEdu.runLiveProcedure('${moduleName.replace(/'/g,"\\'")}')">
                                <i class="fas fa-play me-1"></i> Jalankan Prosedur Validasi
                            </button>
                            <button class="btn btn-light fw-bold px-4 rounded-pill" style="font-size:13px;border:1.5px solid #e8eaed;" onclick="alert('Exporting WTP audit PDF for ${moduleName.replace(/'/g,"\\'")}')">
                                <i class="fas fa-file-pdf me-1 text-danger"></i> Export Audit PDF
                            </button>
                        </div>
                    </div>
                </div>
        ...
        if (window.AtlasFaultGuard) {
            window.AtlasFaultGuard.runIsolated('units', moduleName, 'isolated-unit-modules', renderDetail);
        } else {
            renderDetail();
        }
    }
    ...
    function runLiveProcedure(moduleName) {
        if (window.AtlasFaultGuard) {
            window.AtlasFaultGuard.runIsolated('units', moduleName, 'screen-unit-detail', () => {
                alert(`✓ Validated & executed procedure workflow for [${moduleName}]. COBIT 2019 & ISO 9001 compliance confirmed.`);
            });
        } else {
            alert(`Executed procedure for [${moduleName}]`);
        }
    }
